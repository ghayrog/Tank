...
namespace UserInput
{
    internal class TankAIInput : IUserInput
    {
        ...
        private const float SWITCH_DIRECTION_DELAY = 1f;
        private const float BACKWARDS_MOVEMENT_PROBABILITY = 0.1f;
        private const float STEERING_PROBABILITY = 0.3f;
        ...
        private float _switchDirectionTimer = 0;
        internal TankAIInput(Transform self, string targetTag, float fireMinHold, float fireMaxHold, float shootingDistance)
        {
            _self = self;
        ...
        {
            _switchDirectionTimer += deltaTime;
            if (_switchDirectionTimer >= SWITCH_DIRECTION_DELAY)
            {
                _switchDirectionTimer = 0;
                float directionProbability = Random.Range((float)0, (float)1);
                if (directionProbability <= BACKWARDS_MOVEMENT_PROBABILITY)
                {
                    ForwardMovementAxis = 1;
                }
                else if (directionProbability <= 3 * BACKWARDS_MOVEMENT_PROBABILITY)
                {
                    ForwardMovementAxis = -1;
                }
                else
                {
                    ForwardMovementAxis = 0;
                }
                float steeringProbability = Random.Range((float)0, (float)1);
                if (steeringProbability <= STEERING_PROBABILITY / 2)
                {
                    ChassisRotationAxis = -1;
                }
                else if (steeringProbability <= STEERING_PROBABILITY)
                {
                    ChassisRotationAxis = 1;
                }
                else
                {
                    ChassisRotationAxis = 0;
                }
            }
